         ...
         let f =
           fun acc -> function
             | Ast.Eia.Mul [ Atom (Var v); Pow (Atom (Const 2), Atom (Var _)) ] ->
               S.add acc v
             | Mul [ Atom (Const _); Atom (Var v); Pow (Atom (Const 2), Atom (Var _)) ] ->
               S.add acc v
             | _ -> acc
         in
...
let eq_propagation : Info.t -> Env.t -> Ast.t -> Env.t =
  ...
  let helper info env = function
    ...
    | Eia (Eia.Eq (Mul [ Atom (Const _); Atom (Var v) ], (Atom (Const 0) as rhs)))
      when Env.is_absent_key v env -> Env.extend_exn env v rhs
    | Eia (Eia.Eq (Mul [ Atom (Const cl); Atom (Var v) ], Atom (Const cr)))
      when cr mod cl = 0 && Env.is_absent_key v env ->
      Env.extend_exn env v (Atom (Const (cr / cl)))
    ...
    | Eia (Eia.Eq (Add [ Atom (Var v1); Mul [ Atom (Const c2); Atom (Var v2) ] ], rhs))
      when v1 <> v2 -> single info env 1 v1 c2 v2 rhs
    | Eia (Eia.Eq (Add [ Mul [ Atom (Const c1); Atom (Var v1) ]; Atom (Var v2) ], rhs))
      when v1 <> v2 -> single info env c1 v1 1 v2 rhs
    | Eia
        (Eia.Eq
           ( Add
